...
% % %          %% remove for data after Dec 28,2012 and uncomment the following statements
    ...
    ts=obj(trNo).ts{1, 1};
%     ts=[.513:obj(trNo).FrameTime:size(dffarray,2)*obj(trNo).FrameTime];
...
%     setpt = sesObj.wsArray.wl_trials{trNo}.Setpoint{wskNo};
    setpt = obj(trNo).kappa{1};   
    plot(ts_wsk, setpt,'r','linewidth',2);
...
% %     title(sprintf('Trial %d, ROI %d, wsk %d, %s',trNo, roiNo, wskNo, trType), 'fontsize',20)
     title(sprintf('%d:Trial %s, dFFmean(%s) %s',trNo,soloTrNo,roiName,trType), 'fontsize',20);
...
%     theta = sesObj.wsArray.wl_trials{trNo}.theta{wskNo};
    ...
    set(ha, 'box','off', 'xlim',[min(ts_wsk) max(ts_wsk)])
    figure(gcf)
