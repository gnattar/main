...
function[w_setpoint_trials,w_setpoint_early,w_setpoint_late,w_setpoint_trials_med,w_setpoint_trials_width,w_setpoint_trials_dur,w_setpoint_trials_prewhisk,pvalsetpoint...
    w_amp_trials,w_amp_early,w_amp_late,w_amp_trials_med,w_amp_trials_width,pvalamp]...
...
%         jbfill(trialnums,up,low,[0 .8 .8],[0 .8 .8],1,.5);hold on;
       ...
       plot(trialnums,smoothed,'linewidth',1.5,'color','k');hold on;
        smoothed=filter(ones(1,windowSize)/windowSize,1,setpoint_trials_prewhisk(:,1));
       plot(trialnums,smoothed,'color','r','linewidth',1);       
...
%%plot prewhisk setpointhist        
% %         subplot(numblocks*2,4,(numblocks+blk-1)*4+2);plot(trialnums,setpoint_trials_prewhisk(:,1),'color','k','linewidth',1.5);       
% %         axis([min(trialnums) max(trialnums) -30 30]);
      
%%plot amphist        
%         subplot(numblocks*2,4,(blk-1)*4+4);
...
%         jbfill(trialnums,up',low',[0 .8 .8],[0 .8 .8],1,.5);hold on;
       ...
       plot(trialnums,smoothed,'linewidth',1.5,'color','k');
...
%%plot setpoint first and last n trials
  ...
  %%plot amp first and last n trials
         ...
         w_setpoint_trials_prewhisk(blk)= {setpoint_trials_prewhisk};
