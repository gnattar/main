...
for j = 1: numrois
    ...
    scatter(dKappa_mean( trialsoi),dFF_mean( trialsoi,j),100,col(barposind(trialsoi),:),'o','filled');
...
for j = 1: numrois
    ...
    numtrials = size(trialsoi,2);
    ha(j) = subaxis(numrois,1,j, 'sv', 0.05);
   scatter(theta_med( trialsoi),dFF_mean( trialsoi,j),100,col(barposind(trialsoi),:),'o','filled');
