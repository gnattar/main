...
for i = 1:length(rois)
 subplot(ceil(length(rois)/plotsperrow),plotsperrow,i);imagesc([ts ts(length(ts))+dt*1:dt:ts(length(ts))+dt*5],[1:numtrials],newrois(:,:,rois(i)));caxis(cscale);colormap(jet);
    if(tag_trialtypes ==1)
        vline([1 1.5 2 2.5 ],'k-');
    else    
         vline([.5 ],'r-');
...
%             col= jet(length(types));
            for j=1:length(trials_types)           
             plot(ts ,newrois(trials_types(j),1:length(ts),rois(i))-spacing*(k-1)','color',col(types(k),:),'linewidth',1);           
             hold on;  
            end
            hold on;
        end
             axis([0 ts(length(ts)) (0-spacing*length(types)+100) 400]);
        vline([ 1 1.5 2 2.5],'k-');
     else
        col=linspace(0,1,numtrials);
        for j=1:numtrials         
             plot(ts,newrois(j,:,rois(i))','color',[.7 0 0]*col(j),'linewidth',1);
             axis([0 ts(length(ts)) -50 500]);
             hold on;
        end
        hold off;  
        vline([.5 ],'r-');
...
%                      jbfill(ts,temp_avg+temp_sd,temp_avg-temp_sd,col(types(k),:),col(types(k),:),1,transparency); hold on;
                 ...
                 axis([0 ts(length(ts)) 0-spacing*length(types)+100 300]);
                 vline([ 1 1.5 2 2.5],'k-');
         else
...
%                 jbfill(ts,temp_avg+temp_sd,temp_avg-temp_sd,[.5 0 0 ],[.5 0 0 ],1,transparency); hold on;
                plot(ts ,temp_avg,'color',[.5 0 0 ],'linewidth',1.5);  
    %              plot(ts,newrois(j,:,rois(i))','color',col(j,:),'linewidth',1);
                 axis([0 ts(length(ts)) -20 300]);
