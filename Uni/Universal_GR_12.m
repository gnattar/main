...
ind = strfind(datapath,'roi');
roiname = ['R_' datapath(ind+3)];
ind = strfind(datapath,'rawdata');
% CaSignal.results_path  = [datapath(1:ind-1) filesep 'results'];
CaSignal.results_path  = datapath;
...
CaSignal.results_fn = [CaSignal.results_path filesep 'CaSignal_CaTrials_' FileName_prefix roiname '.mat'];
CaSignal.results_roiinfo = [CaSignal.results_path filesep 'ROIinfo_', FileName_prefix roiname '.mat'];
...
if get(hObject, 'Value') == 1
    ...
    if isfield(CaSignal, 'ica_data') && isfield(CaSignal.ica_data,'Data')%%~isempty(CaSignal.ica_data.Data)
        ...
        CaSignal.ica_data.FileNums = [1:60];% using filenums [1:60] for now , need to put a text box
        usr_confirm = questdlg('ica_data.Data exists. Reload data from first 60 data .tif files.Continue? ');
        if strcmpi(usr_confirm, 'Yes')
            % Load the data files
        ...
        CaSignal.ica_data.FileBaseName = get(handles.batchPrefixEdit, 'String');
        CaSignal.ica_data.DataDir = pwd;
        CaSignal.ica_data.FileNums = [1:50];% using filenums [1:60] for now , need to put a text box
        % Load the data files
        CaSignal.ica_data.Data = ICA_LoadData( CaSignal.ica_data.DataDir,  CaSignal.ica_data.FileBaseName,  CaSignal.ica_data.FileNums);
...
elseif(get(handles.sorted_CaTrials_select,'Value') ==1)
    ...
    javaaddpath('/Users/ranganathang/Documents/MATLAB/main/helper_funcs/jheapcl/jheapcl/MatlabGarbageCollector.jar');
    jheapcl(1)
...
%% removing all nogo trials
nogotrials = find(Y > 2);
trialinds (nogotrials) = [];
trialnums(nogotrials) = [];
wSig_tags(nogotrials)=[];
CaSig_tags(nogotrials)=[];
Y(nogotrials)=[];
...
for j= 1:numblocks
    ...
    saveas(gcf,['allsessions_thetaenv_med_bl ' blocklist{j}] ,'jpg');
    ...
    saveas(gcf,['allsessions_error_bl ' blocklist{j}] ,'jpg');
...
for j= 1:numblocks
    ...
    axis([0 count 0 maxdata+.02]);grid on; ylabel('Percent dwell past mena barpos'); xlabel('Trials');
    saveas(gcf,['allsessions_prcpastbarpos_bl ' blocklist{j}] ,'jpg');
...
for j= 1:numblocks
    ...
    title([commentstr 'Mean Thetaenv past meanbarpos' ]); %' blocklist{j}]);
    ...
    for i = 1:numsessions
        ...
        selecteddata = strrep(datatoplot,'data','meanbarcross');
        temp = getfield(wSigSummary{i},selecteddata);
        temp = temp{1};
        curr_meanbarpos=  wSigSummary{i}.nogo_thetaenv_mean_barpos{1}{1};
        binnedxdata = temp{block}(:,1)';
    ...
    axis([0 count mindata-5 maxdata+10]);grid on; ylabel('Mean thetaenv past barpos'); xlabel('Trials');
    saveas(gcf,['allsessions_meanpastbarpos_bl ' blocklist{j}] ,'jpg');
...
while(count>=0)
    [filename,pathName]=uigetfile('wSigSummary*.mat','Load wSigSummary.mat file');
    if isequal(filename, 0) || isequal(pathName,0)
        break
    end
    count=count+1;
...
mean_subtract_on =1;
count =1;
fieldnames = {'nogo_thetaenv_medbinned';'nogo_thetaenv_prepolebinned';'nogo_thetaenv_peakbinned';'nogo_thetaenv_prcpastmeanbarbinned'};
for k = 1:length(fieldnames)
    if(strcmp(fieldnames(k),'nogo_thetaenv_prcpastmeanbarbinned'))
        mean_subtract_on = 0;
    end    
    [tempobj,propname,sess_count] = sort_SessionData(wSigSum_anm,fieldnames{k},mean_subtract_on);
...
for k = 1:length(fieldnames)  
    ...
    for i = 1:numanm
        ...
        if mean_subtract_on
            title ([propname ' plot Norm. Change over T50 to T250 ']);
        else
             title ([propname ' plot Prc occupancy change over T50 to T250 ']);
        end
    end
...
function [tempobj,propname,sess_count] = sort_SessionData(wSigSum_anm,fieldname,mean_subtract_on)
    ...
    for i = 1:numanm
        ...
        for j= 1:numsessions
            ...
            target_barpos = curr_sess.nogo_thetaenv_biased_barpos{1};
...
numcolstoplot  = 5+1; % im + 5 go pos % CaSigSummary{1,1}
types = [1:5];
...
for j = 1:numsessions
    ...
    trialvar = cell2mat(arrayfun(@(x) x.trialtype, obj, 'uniformoutput',false));
    barpositions1 = unique(barposall);
    nogotouchtrials = sum(strcmp(trialvar ,'CR'));
    nogotouchtrials = nogotouchtrials + sum(strcmp(trialvar ,'FA')); 
    if(nogotouchtrials>0)
        str =2;
    end
...
%     temp2=temp2*(1/length(unique(trialtypes))) *cscale(1,2);
    ...
    current_types = length(unique(trialtypes));
    if(current_types >types )
        ('Error in number of trial types : may have more than 5 go positions CHECK!!')
            break
%         types = current_types;
%         numcolstoplot = current_types+1;
%         skip =0;
    elseif(current_types < types )
        numcolstoplot = types +1;
        skip =setdiff(types,current_types);        
    end
...
% %         vline([.5 1 1.5 2 2.5 ],'k-');
        ...
        count=count+1;
        current_types= unique(trialtypes);
...
% %             vline([ .5 1 1.5 2 2.5],'k-');
            ...
            [all_data,detected] = detect_Ca_events(temp_data,frametime,60);
            ...
            hold on;
            u = 120;
            axis([0 ts(length(ts)) -10 u]);set(gca,'YMinorTick','on','YTick', -200:100:u);xlabel('Time(s)'); ylabel('mean_dFF');           
            vline([.5 1 1.5 2 2.5],'k-');
            txtbox = text(2.0,u-100,[ num2str(sum(detected,1)) '/' num2str(size(all_data,1)) '(' num2str(round( (sum(detected,1)/size(all_data,1))*100)/100) ')'],'FontSize',13);
...
%             axis([min(temp_totalTouchKappa)-1 max(temp_totalTouchKappa)+1 -10 max(max_dFF) ]); set(gca,'YMinorTick','on','YTick', 0:100:1000);
            axis([-1 30 -10 u ]); set(gca,'YMinorTick','on','YTick', 0:100:u);
            txtbox = text(20, u-20 , [num2str(round(P(1)*100)/100)],'FontSize',13);
...
cd (datapath);
set (handles.contact_Sig_datapath,'String',datapath);
load(fname,'-mat');
