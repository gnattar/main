...
rois_name_tag = '';
    ...
    % % %          if(tag_trialtypes ==1)         
                ...
                for k = 1:length(types)
                    ...
                    axis([0 ts(length(ts)) -100 600]);set(gca,'YMinorTick','on','YTick', -100:100:600);
                    vline([ .5 1 1.5 2 2.5],'k-');
                end
...
%                     plot([frametime:frametime:length(detected_avg)*frametime] ,detected_avg,'color',col(types(k),:),'linewidth',1.5);           
                   ...
                   axis([0 ts(length(ts)) -10 300]);set(gca,'YMinorTick','on','YTick', -100:100:400);xlabel('Time(s)'); ylabel('mean_dFF');
...
%                           set(gca,'Xscale','log');
                          ...
                          axis([-5 35 -10 600 ]); set(gca,'YMinorTick','on','YTick', 0:100:600);
    %                     vline([.5 1 1.5 2 2.5],'k-');
