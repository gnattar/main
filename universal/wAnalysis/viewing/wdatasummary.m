...
shoulder = 10; % avg 5 deg between pole positions , should = 2.5 deg
...
for(blk=1:numblocks)
    ...
    for(i=1:1:length(trialnums))
        ...
        if(frames<1500)
            ...
            past_meanbar= find((biased_bartheta+shoulder > thetaenv_bins) & (thetaenv_bins > biased_bartheta - shoulder));
            ...
            thetaenv_peak(i,1) = mean(temp); %% 
            thetaenv_peak(i,2) = std(temp);
            
            thetaenv_prcpastmeanbar(i,1) = sum(thetaenv_dist(i,past_meanbar));
            temp = thetaenv_pole((biased_bartheta+shoulder>thetaenv_pole) &  (thetaenv_pole> biased_bartheta- shoulder));
            if ~isempty(temp)
...
% % %               plot(thetaenv_bins(1,:),thetaenv_dist(p,:),'linewidth',1.5); grid on;hold on;line([thetaenv_meanbarcross(p,1)'; thetaenv_meanbarcross(p,1)'], [ones(1,1)*.05; ones(1,1)*.15],'linewidth',1.5);hold off; title(['Dist of trial' num2str(p) str timewindowtag]);saveas (gcf,[fpath,filesep,'Dist' num2str(p)],'jpg');
% % %             end
% % %            close (h1c);
% % %         end
