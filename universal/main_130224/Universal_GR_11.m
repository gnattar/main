...
function [gopos,nogopos]=Batch_whiskers_preprocess(basedatapath,coordsatfirst,coordsatnext,barposatfirst,barposatnext)
    ...
    sessionInfo = struct('pxPerMm', 31.25, 'mouseName', solo_obj.mouseName,'sessionName',solo_obj.sessionName(end-6:end),'SoloTrialStartEnd',obj.trialStartEnd,'whiskerPadOrigin_nx',[375,70],'whiskerImageDim',[520,400],...
        'bar_coords',[],'bar_time_window',[1.0827,2.5828],'whisker_trajIDs',[0],'theta_kappa_roi',[0,20,140,170],'gotrials',[],'nogotrials',[],'gopos',[],'nogopos',[],'gopix',[],'nogopix',[]);
...
%     nogotrialstoplot=[];
    ...
    for k=1:length(blocks.nogotrialnames{i})
     ...
     timewindowtag = get(handles.timewindowtag,'String');
     [w_setpoint_trials,w_setpoint_early,w_setpoint_late,w_setpoint_trials_med,w_setpoint_trials_width,w_setpoint_trials_dur,w_setpoint_trials_prewhisk,pvalsetpoint,...
         w_amp_trials,w_amp_early,w_amp_late,w_amp_trials_med,w_amp_trials_width,pvalamp]= wdatasummary(wSigTrials,blocks.tag,blocks.nogotrialnums,avg_trials,gopix,nogopix,restrictTime,p,plot_whiskerfits,'nogo',timewindowtag);
     for i=1:numblocks
         blocks.nogo_setpoint_trials{i} =  w_setpoint_trials{i};
         blocks.nogo_setpoint_early{i}=w_setpoint_early{i};
         blocks.nogo_setpoint_late{i}=w_setpoint_late{i};
         blocks.nogo_setpoint_trials_med{i} = w_setpoint_trials_med{i};
         blocks.nogo_setpoint_trials_width{i} = w_setpoint_trials_width{i};
         blocks.nogo_setpoint_trials_dur{i} =w_setpoint_trials_dur{i};
         blocks.nogo_setpoint_trials_prewhisk{i} =w_setpoint_trials_prewhisk{i};
         blocks.nogo_pvalsetpoint = pvalsetpoint;
         blocks.nogo_amp_trials{i} =  w_amp_trials{i};
         blocks.nogo_amp_early{i}=w_amp_early{i};
         blocks.nogo_amp_late{i}=w_amp_late{i};
         blocks.nogo_amp_trials_med{i} = w_amp_trials_med{i};
         blocks.nogo_amp_trials_width{i} = w_amp_trials_width{i};
%          blocks.nogo_amp_trials_hist{i} =w_amp_trials_hist{i};
         blocks.nogo_pvalamp = pvalamp;
     end
     [w_setpoint_trials,w_setpoint_early,w_setpoint_late,w_setpoint_trials_med,w_setpoint_trials_width,w_setpoint_trials_dur,w_setpoint_trials_prewhisk,pvalsetpoint,...
         w_amp_trials,w_amp_early,w_amp_late,w_amp_trials_med,w_amp_trials_width,pvalamp]= wdatasummary(wSigTrials,blocks.tag,blocks.gotrialnums,avg_trials,gopix,nogopix,restrictTime,p,plot_whiskerfits,'go',timewindowtag);
     for i=1:numblocks
         blocks.go_setpoint_trials{i} =  w_setpoint_trials{i};
         blocks.go_setpoint_early{i} = w_setpoint_early{i};
          blocks.go_setpoint_late{i} = w_setpoint_late{i};
         blocks.go_setpoint_trials_med{i} = w_setpoint_trials_med{i};
         blocks.go_setpoint_trials_width{i} = w_setpoint_trials_width{i};
         blocks.go_setpoint_trials_dur{i} =w_setpoint_trials_dur{i};
         blocks.go_setpoint_trials_prewhisk{i} =w_setpoint_trials_prewhisk{i};
         blocks.go_pvalsetpoint = pvalsetpoint;
         blocks.go_amp_trials{i} =  w_amp_trials{i};
         blocks.go_amp_early{i} = w_amp_early{i};
         blocks.go_amp_late{i} = w_amp_late{i};
         blocks.go_amp_trials_med{i} = w_amp_trials_med{i};
         blocks.go_amp_trials_width{i} = w_amp_trials_width{i};
%          blocks.go_amp_trials_hist{i} = w_amp_trials_hist{i};
         blocks.go_pvalamp = pvalamp;
     end 
     fpath = pwd;
     ...
     save(fname,'blocks');
% % %    [wAmpm,wAmpsd]= plotdetails(wSigTrials,nogotrials(nogotrialstoplot),gopix,nogopix,blocks,tags,restrictTime,p,plot_whiskerfits); 
% % % % % % 
...
%             extractedTheta(1:endp-str)= temp(contactind(j)-ceil(baseline*wSigframerate):contactind(j)+ceil(dur*wSigframerate));
            ...
            %temporary fix for delay in whisker data
%             timepoint=timepoint+.5;
            if(timepoint<=.513)
                strframe=1;
                diff=ceil((0.513-timepoint)/Caframetime);
            else
                strframe=ceil((timepoint-0.513)/Caframetime);
            end
...
function contactdetect_Callback(hObject, eventdata, handles)
    ...
    contDet_param.threshDistToBarCenter = [0   0.7];
...
% % % figure;
% % % plot(solo_data.hitTrialNums,zeros(1,length(solo_data.hitTrialNums)),'g.','MarkerSize',10); hold on;
% % % plot(solo_data.missTrialNums,zeros(1,length(solo_data.missTrialNums)),'r.','MarkerSize',10); hold on;
% % % plot(solo_data.falseAlarmTrialNums,ones(1,length(solo_data.falseAlarmTrialNums)),'r.','MarkerSize',10); hold on;
% % % plot(solo_data.correctRejectionTrialNums,ones(1,length(solo_data.correctRejectionTrialNums)),'g.','MarkerSize',10); hold off;
% % % title(['Performance from ' name(length(name)-16:length(name)-8) ' Session'  name(length(name)-6:length(name)-1)]);
% % % axis([0 length(solo_data.trialNums) -1 2]);
% set(gcf,) ;
% % % saveas(gcf,'solo_behavior','tif');
...
for j= 1:numblocks
    block =j;
    sc = get(0,'ScreenSize');
    h_fig1 = figure('position', [1000, sc(4)/10-100, sc(3)*3/10, sc(4)*3/4], 'color','w'); %%raw theta
    ah1=axes('Parent',h_fig1);
    title([commentstr 'Theta Block ' blocklist{j} 'Data ' datatoplot]);
    h_fig2 = figure('position', [300, sc(4)/10-100, sc(3)*3/10, sc(4)*3/4], 'color','w'); %% error from bartheat
    ah2=axes('Parent',h_fig2); title([commentstr 'Error Block ' blocklist{j} 'Data ' datatoplot]);
    hold on;
%     figure;
    count =0;
    prev=0;
    ...
    for i = 1:numsessions
       ...
       if(size(temp{block},2)<2)
           lower = data-temp{block}(:,1)';
           upper = data+temp{block}(:,1)';
       else
           lower = temp{block}(:,1)';
           upper = temp{block}(:,2)';
       
           minedge = temp{block}(:,3)';
           maxedge = temp{block}(:,4)';
       end
       ...
       %smoothing over 25 trials
       windowSize=25;
       data=filter(ones(1,windowSize)/windowSize,1,data);
        upper=filter(ones(1,windowSize)/windowSize,1,upper);
        lower=filter(ones(1,windowSize)/windowSize,1,lower);     
        lower(isnan(lower))=0;
        upper(isnan(upper))=0;
        data(isnan(data))=0;
      axes(ah1);  
