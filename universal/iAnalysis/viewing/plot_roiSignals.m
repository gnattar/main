...
%                     plot([frametime:frametime:length(detected_avg)*frametime] ,detected_avg,'color',col(types(k),:),'linewidth',1.5);           
                   ...
                   axis([0 ts(length(ts)) -50 250]);set(gca,'YMinorTick','on','YTick', -50:50:300);xlabel('Time(s)'); ylabel('mean_dFF');

                    vline([.5 1 1.5 2 2.5],'k-');
                    text(2,100,[ num2str(sum(detected,1)) '/' num2str(size(event_detected_data,1)) '(' num2str(sum(detected,1)/size(event_detected_data,1)) ')']);%,'Location','NorthEast');
