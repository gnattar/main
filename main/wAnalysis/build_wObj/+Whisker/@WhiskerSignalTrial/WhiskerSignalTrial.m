classdef WhiskerSignalTrial < handle
    ...
    properties
        ...
        %                      values in R to NaN.
        maskTreatment = 'none';%'maskNaN'; % Alternatively, {'maskNaN','none','mask'} to set three (or more)
...
%             end
        ...
        function [t,theta,kappa] = mean_theta_and_kappa(obj,tid,radial_window_theta,radial_window_kappa)
            ...
            t = obj.get_time(tid);
            infall=find(diff(t)==0); 
           
%             
%             if~isempty(infall)   
%               t(infall)=mean([t(infall-1)' t(infall+1)'],2)';         
%             end
            ...
            if sum(missing)==length(kappa)
                disp(['Err in kappa, Delete this trial: ' obj.trackerFileName])
                disp('Less than 2 non-NaN values for mean kappa timeseries. May be a bad mask, that no whiskers cross.')
            end
            try
            ...
            if sum(missing)==length(theta)
                disp(['Err in theta, Delete this trial: ' obj.trackerFileName])
                disp('Less than 2 non-NaN values for mean theta timeseries. May be a bad mask, that no whiskers cross.')
            end
...
%             [tmp1,thetaContact,tmp2,yContact,xContact,distanceToPoleCenter,t] = obj.get_r_theta_kappa_nearest_bar(tid);
    ...
    methods % Dependent property methods; cannot have attributes.
        ...
        function obj = set.barPosClean(obj,val)
            obj.barPos = val;
        end
